...
VerifyResult AndroidCertificateVerifier::verify(
    std::string_view hostname, const std::vector<DerCertificate> &chain,
    std::string_view auth_type) {
  if (!ext_) {
    return {false, KATHTTP_ERR_NO_TRUST_PROVIDER,
            "no Android trust manager available"};
  }
  JNIEnv *env = nullptr;
  bool attached = false;
  if (vm_->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6) != JNI_OK) {
  ...
  VerifyResult result{false, KATHTTP_ERR_CERTIFICATE_VERIFY,
  ...
  jmethodID cf_get = cf_cls ? env->GetStaticMethodID(
      cf_cls, "getInstance", "(Ljava/lang/String;)Ljava/security/cert/CertificateFactory;") : nullptr;
  jmethodID bais_ctor = bais_cls ? env->GetMethodID(bais_cls, "<init>", "([B)V") : nullptr;
  jmethodID gen_cert = cf_cls ? env->GetMethodID(cf_cls, "generateCertificate", "(Ljava/io/InputStream;)Ljava/security/cert/Certificate;") : nullptr;
  if (x509_cls && cf_get && bais_ctor && gen_cert) {
    ...
    if (cf) {
      jobjectArray arr =
          env->NewObjectArray(static_cast<jsize>(chain.size()), x509_cls,
                               nullptr);
      if (arr) {
        ...
        jclass ext_cls =
            env->FindClass("android/net/http/X509TrustManagerExtensions");
        if (ext_cls) {
          jmethodID check = env->GetMethodID(
              ext_cls, "checkServerTrusted",
          ...
          if (check) {
            ...
            if (env->ExceptionCheck()) {
              ...
              if (ex) {
                ...
                jmethodID to_string =
                    env->GetMethodID(ex_cls, "toString",
                                      "()Ljava/lang/String;");
                jstring msg = to_string
                                     ? static_cast<jstring>(
                                           env->CallObjectMethod(ex, to_string))
                                     : nullptr;
                if (msg) {
...
CertificateVerifier *create_android_platform_verifier(JavaVM *vm) {
  ...
  jmethodID get_def = env->GetStaticMethodID(
      tmf_cls, "getDefaultAlgorithm", "()Ljava/lang/String;");
  ...
  jstring algo =
      static_cast<jstring>(env->CallStaticObjectMethod(tmf_cls, get_def));
  ...
  jmethodID get_tms = env->GetMethodID(
      tmf_cls, "getTrustManagers", "()[Ljavax/net/ssl/TrustManager;");
  if (!init || !get_tms) return nullptr;
  env->CallVoidMethod(tmf, init, nullptr);
  jobjectArray tms =
      static_cast<jobjectArray>(env->CallObjectMethod(tmf, get_tms));
  ...
  jmethodID ext_ctor =
      env->GetMethodID(ext_cls, "<init>", "(Ljavax/net/ssl/X509TrustManager;)V");
