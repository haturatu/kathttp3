...
public:
  ...
  bool init(kathttp_trust_mode trust_mode, bool insecure,
            const std::string &ca_cert_file, const std::string &keylog_file,
            CertificateVerifier *platform_verifier = nullptr);

  SSL_CTX *native() const { return ssl_ctx_; }
  CertificateVerifier *verifier() const { return verifier_; }
...
public:
  ...
  std::string lastTlsError() const { return last_failure_.message; }

  /* The captured failure (valid after a verification / handshake error). */
  const TlsFailure &lastFailure() const { return last_failure_; }
  ...
  void resetFailure() { last_failure_ = TlsFailure{}; }
