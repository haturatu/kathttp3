...
bool load_embedded_ca_bundle(SSL_CTX *ssl_ctx) {
  ...
  BIO *bio = BIO_new_mem_buf(kCaBundlePem,
                               static_cast<int>(sizeof(kCaBundlePem) - 1));
  if (!bio) return false;
  X509 *x;
  bool added = false;
  while ((x = PEM_read_bio_X509_AUX(bio, nullptr, nullptr, nullptr)) !=
         nullptr) {
...
std::vector<DerCertificate> peer_chain_der(SSL *ssl) {
  ...
  if (!chain) return out;
  for (size_t i = 0; i < static_cast<size_t>(sk_CRYPTO_BUFFER_num(chain));
       ++i) {
...
enum ssl_verify_result_t cert_verify_cb(SSL *ssl, uint8_t *out_alert) {
  *out_alert = 42 /* bad_certificate */;
  ...
  auto *s = static_cast<TlsClientSession *>(
      SSL_get_ex_data(ssl, g_session_ex_index));
...
bool TlsClientContext::init(kathttp_trust_mode trust_mode, bool insecure,
  ...
  if (trust_mode == KATHTTP_TRUST_PLATFORM) {
    CertificateVerifier *v =
        platform_verifier ? platform_verifier : platform_cert_verifier();
    if (v) {
      verifier_ = v;  // injected by the host; not owned here
    } else {
      owned_verifier_ =
          make_verifier(KATHTTP_TRUST_EMBEDDED_MOZILLA, false, "");
...
void TlsClientSession::recordVerifierResult(const VerifyResult &r) {
  last_failure_ = TlsFailure{
      ...
      0,
      0,
      r.ok ? 0 : (r.code ? r.code : KATHTTP_ERR_CERTIFICATE_VERIFY),
