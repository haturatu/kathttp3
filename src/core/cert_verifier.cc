...
VerifyResult verify_with_store(const std::vector<DerCertificate> &chain,
                              X509_STORE *store, std::string_view hostname) {
  if (!store) {
  ...
  if (ctx && X509_STORE_CTX_init(ctx, store, certs[0], untrusted) == 1) {
    ...
    if (X509_verify_cert(ctx) == 1) {
      ...
      if (e == X509_V_ERR_HOSTNAME_MISMATCH) {
        result = {false, KATHTTP_ERR_HOSTNAME_MISMATCH,
                  "certificate hostname mismatch"};
      } else {
        const char *reason =
            X509_verify_cert_error_string(static_cast<long>(e));
...
public:
  ...
  VerifyResult verify(std::string_view hostname,
...
public:
  EmbeddedBundleCertificateVerifier()
      : StoreCertificateVerifier(
            store_from_pem(kCaBundlePem, sizeof(kCaBundlePem) - 1)) {}
};
...
CertificateVerifier *platform_cert_verifier() { return g_platform_verifier; }

/* Factory used by the TLS layer once the trust policy is known. */
std::unique_ptr<CertificateVerifier> make_verifier(kathttp_trust_mode mode,
                                                  bool insecure,
